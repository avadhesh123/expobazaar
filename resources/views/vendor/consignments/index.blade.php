...
<div class="card">
    ...
    <div class="card-body" style="padding:0;overflow-x:auto;">
        <table class="data-table">
            <thead><tr><th>Consignment #</th><th>Country</th><th>Items</th><th>CBM</th><th>Live Sheet</th><th>GRN</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($c->grn)<span class="badge badge-success"><i class="fas fa-check"></i> {{ $c->grn->grn_number }}</span>
                        @else<span style="font-size:.72rem;color:#94a3b8;">—</span>@endif
                    </td>
                    ...
                    <td>
                        <div style="display:flex;gap:.25rem;flex-wrap:wrap;">
                            ...
                            <button type="button" class="btn btn-outline btn-sm" onclick="document.getElementById('insp{{ $c->id }}').style.display=document.getElementById('insp{{ $c->id }}').style.display==='none'?'table-row':'none'"><i class="fas fa-file-upload"></i> Inspection</button>
                                                   <button type="button" class="btn btn-outline btn-sm" onclick="document.getElementById('cinv{{ $c->id }}').style.display=document.getElementById('cinv{{ $c->id }}').style.display==='none'?'table-row':'none'"><i class="fas fa-file-upload"></i>  Commercial Invoice and Packing List</button>
 </div>
                    </td>
                </tr>
                {{-- Inspection Upload --}}
                <tr id="insp{{ $c->id }}" style="display:none;background:#eff6ff;">
                    <td colspan="8" style="padding:.75rem;">
                        <form method="POST" action="{{ route('vendor.inspections.upload', $c) }}" enctype="multipart/form-data" style="display:flex;flex-wrap:wrap;gap:.5rem;align-items:flex-end;">@csrf
                            ...
                            <div><label style="font-size:.65rem;font-weight:600;color:#1e40af;">Result</label><select name="result" style="padding:.3rem .5rem;border:1px solid #bfdbfe;border-radius:6px;font-size:.82rem;font-family:inherit;"><option value="">Select...</option><option value="pass">Pass</option><option value="fail">Fail</option><option value="conditional">Conditional</option></select></div>
                ...
                {{-- Commercial Invoice and Packing List Upload --}}
                <tr id="cinv{{ $c->id }}" style="display:none;background:#eff6ff;">
                    <td colspan="8" style="padding:.75rem;">
                        <form method="POST" action="{{ route('vendor.commercial-invoices.upload', $c) }}" enctype="multipart/form-data" style="display:flex;flex-wrap:wrap;gap:.5rem;align-items:flex-end;">@csrf
                            <div><label style="font-size:.65rem;font-weight:600;color:#1e40af;">Commercial Invoice *</label><input type="file" name="commercial_invoice" required style="font-size:.78rem;"></div>
                            <div><label style="font-size:.65rem;font-weight:600;color:#1e40af;">Packing List *</label><input type="file" name="packing_list" required style="font-size:.78rem;"></div>
                            <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-upload"></i> Upload</button>
                        </form>
                    </td>
                </tr>
