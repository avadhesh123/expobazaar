    ...
    <div class="card-body" style="padding:1rem 1.4rem;">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:.75rem;">
            ...
            <div style="padding:.5rem;background:#f8fafc;border-radius:8px;">
                ...
                <div style="font-weight:600;">{{ $liveSheet->offerSheet->offer_sheet_number ?? '—' }}</div>
            </div>
            <div style="padding:.5rem;background:#f8fafc;border-radius:8px;">
...
<div class="card" style="margin-bottom:1.25rem;border-color:#16a34a;">
    <div class="card-header" style="background:#f0fdf4;">
        <h3 style="color:#166534;"><i class="fas fa-file-upload" style="margin-right:.5rem;"></i> Upload Filled Live Sheet</h3>
    </div>
    <div class="card-body">
...
<form method="POST" action="{{ route('vendor.live-sheets.submit', $liveSheet) }}">
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fas fa-list" style="margin-right:.5rem;color:#e8a838;"></i> Product Details ({{ $liveSheet->items->count() }})</h3>
        </div>
...
<script>
    function calcRow(idx) {
        var q = parseFloat(document.querySelector('[name="items[' + idx + '][quantity]"]').value) || 0,
            p = parseFloat(document.querySelector('[name="items[' + idx + '][unit_price]"]').value) || 0,
            c = parseFloat(document.querySelector('[name="items[' + idx + '][cbm_per_unit]"]').value) || 0,
            w = parseFloat(document.querySelector('[name="items[' + idx + '][weight_per_unit]"]').value) || 0;
        document.getElementById('totalPrice' + idx).textContent = '$' + (q * p).toFixed(2);
        document.getElementById('totalCbm' + idx).textContent = (q * c).toFixed(3);
        document.getElementById('totalWeight' + idx).textContent = (q * w).toFixed(2) + ' kg';
        recalcGrand();
    }
    ...
    function recalcGrand() {
        ...
        rows.forEach(function(el, i) {
            var q = parseFloat(el.value) || 0,
                p = parseFloat(document.querySelector('[name="items[' + i + '][unit_price]"]').value) || 0,
                c = parseFloat(document.querySelector('[name="items[' + i + '][cbm_per_unit]"]').value) || 0,
                w = parseFloat(document.querySelector('[name="items[' + i + '][weight_per_unit]"]').value) || 0;
            tq += q;
            tp += q * p;
            tc += q * c;
            tw += q * w;
