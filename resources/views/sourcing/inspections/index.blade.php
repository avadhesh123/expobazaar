...
<div style="display:flex;gap:1rem;margin-bottom:1.25rem;">
    <div class="kpi-card" style="flex:1;"><div class="kpi-label">Total Reports</div><div class="kpi-value">{{ $stats['total'] }}</div></div>
    <div class="kpi-card" style="flex:1;border-left:3px solid #3b82f6;"><div class="kpi-label">Inline</div><div class="kpi-value" style="color:#3b82f6;">{{ $stats['inline'] }}</div></div>
    <div class="kpi-card" style="flex:1;border-left:3px solid #e8a838;"><div class="kpi-label">Midline</div><div class="kpi-value" style="color:#e8a838;">{{ $stats['midline'] }}</div></div>
    <div class="kpi-card" style="flex:1;border-left:3px solid #7c3aed;"><div class="kpi-label">Final</div><div class="kpi-value" style="color:#7c3aed;">{{ $stats['final'] }}</div></div>
...
<div class="card" style="margin-bottom:1.25rem;border-color:#e8a838;">
    <div class="card-header" style="background:#fffbeb;"><h3 style="color:#92400e;"><i class="fas fa-upload" style="margin-right:.5rem;"></i> Upload Inspection Report</h3></div>
    <div class="card-body">
...
<div class="card" style="margin-bottom:1.25rem;">
    <div class="card-body" style="padding:.85rem 1.4rem;">
        <form method="GET" action="{{ route('sourcing.inspections') }}" style="display:flex;gap:.75rem;align-items:flex-end;">
            <div style="min-width:130px;"><label style="font-size:.7rem;font-weight:600;color:#64748b;display:block;margin-bottom:.25rem;">Type</label><select name="type" style="width:100%;padding:.4rem .5rem;border:1px solid #d1d5db;border-radius:8px;font-size:.82rem;"><option value="">All</option><option value="inline" {{ request('type')==='inline'?'selected':'' }}>Inline</option><option value="midline" {{ request('type')==='midline'?'selected':'' }}>Midline</option><option value="final" {{ request('type')==='final'?'selected':'' }}>Final</option></select></div>
            <div style="min-width:130px;"><label style="font-size:.7rem;font-weight:600;color:#64748b;display:block;margin-bottom:.25rem;">Result</label><select name="result" style="width:100%;padding:.4rem .5rem;border:1px solid #d1d5db;border-radius:8px;font-size:.82rem;"><option value="">All</option><option value="passed" {{ request('result')==='passed'?'selected':'' }}>Passed</option><option value="failed" {{ request('result')==='failed'?'selected':'' }}>Failed</option><option value="conditional" {{ request('result')==='conditional'?'selected':'' }}>Conditional</option></select></div>
...
{{-- All Reports --}}
<div class="card">
    <div class="card-header"><h3><i class="fas fa-clipboard-check" style="margin-right:.5rem;color:#1e3a5f;"></i> All Inspection Reports</h3></div>
    <div class="card-body" style="padding:0;overflow-x:auto;">
        <table class="data-table">
            <thead><tr><th>Consignment</th><th>Vendor</th><th>Type</th><th>Report</th><th>Result</th><th>Remarks</th><th>Uploaded By</th><th>Date</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                @empty
                <tr><td colspan="9" style="text-align:center;padding:3rem;color:#94a3b8;"><i class="fas fa-clipboard-check" style="font-size:2rem;display:block;margin-bottom:.5rem;"></i>No inspection reports yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
