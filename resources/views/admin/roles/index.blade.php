        ...
        <div class="card">
            ...
            <div class="card-body" style="padding:0;">
                <table class="data-table">
                    <thead>
                        <tr><th>Role</th><th>Users</th><th>Permissions</th><th>Company Codes</th><th style="width:120px;">Actions</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr onclick="showRoleDetail({{ $role->id }})" style="cursor:pointer;">
                            ...
                            <td onclick="event.stopPropagation();">
                                <div style="display:flex;gap:.3rem;">
                                    <a href="{{ route('admin.roles.edit', $role) }}" class="btn btn-outline btn-sm" title="Edit Role"><i class="fas fa-edit"></i></a>
                                    @if($role->users()->count() === 0)
                                    <form method="POST" action="{{ route('admin.roles.delete', $role) }}" style="display:inline;" onsubmit="return confirm('Delete role \'{{ $role->display_name ?? $role->name }}\'?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm" title="Delete Role"><i class="fas fa-trash"></i></button>
                                    </form>
                                    @endif
                                </div>
                            </td>
                        ...
                        @empty
                        <tr><td colspan="5" style="text-align:center;padding:2rem;color:#94a3b8;">No roles defined yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
