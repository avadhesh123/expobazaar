...
<div class="card" style="margin-bottom:1.25rem;">
    <div class="card-body" style="padding:1.25rem 1.4rem;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(155px,1fr));gap:.75rem;">
            ...
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">Destination</div>
                <div style="font-weight:700;">{{ $flags[$shipment->destination_country] ?? '' }} {{ $shipment->destination_country }}</div>
            </div>
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">Total CBM</div>
                <div style="font-weight:800;font-family:monospace;font-size:1rem;">{{ number_format($shipment->total_cbm, 2) }}</div>
            </div>
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                ...
                <div style="height:6px;background:#e2e8f0;border-radius:3px;margin-top:.2rem;">
                    <div style="height:100%;width:{{ min($utilPct,100) }}%;border-radius:3px;background:{{ $utilPct > 100 ? '#dc2626' : ($utilPct > 85 ? '#e8a838' : '#16a34a') }};"></div>
                </div>
            </div>
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">Items</div>
                <div style="font-weight:700;">{{ $shipment->total_items }}</div>
            </div>
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">Value</div>
                <div style="font-weight:700;">${{ number_format($shipment->total_value, 2) }}</div>
            </div>
            <div style="padding:.65rem;background:{{ $shipment->sailing_date ? '#dcfce7' : '#fef3c7' }};border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">Sailing Date</div>
                <div style="font-weight:700;">{{ $shipment->sailing_date?->format('d M Y') ?? 'Not set' }}</div>
            </div>
            <div style="padding:.65rem;background:#f8fafc;border-radius:8px;">
                <div style="font-size:.62rem;color:#64748b;text-transform:uppercase;font-weight:600;">ETA</div>
                <div style="font-weight:600;">{{ $shipment->eta_date?->format('d M Y') ?? '—' }}</div>
            </div>
        </div>
        ...
        <div style="margin-top:1rem;padding:.75rem;background:#eff6ff;border-radius:8px;display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:.5rem;">
            ...
            @if($shipment->container_number)<div><span style="font-size:.65rem;color:#1e40af;font-weight:600;">Container Number:</span><br><span style="font-weight:600;">{{ $shipment->container_number }}</span></div>@endif
            @if($shipment->sailing_date)<div><span style="font-size:.65rem;color:#1e40af;font-weight:600;">Sailing Date:</span><br><span style="font-weight:600;">{{ $shipment->sailing_date->format('d M Y') }}</span></div>@endif
            @if($shipment->eta_date)<div><span style="font-size:.65rem;color:#1e40af;font-weight:600;">ETA Date:</span><br><span style="font-weight:600;">{{ $shipment->eta_date->format('d M Y') }}</span></div>@endif
            @if($shipment->arrival_date)<div><span style="font-size:.65rem;color:#1e40af;font-weight:600;">Arrival Date:</span><br><span style="font-weight:600;">{{ $shipment->arrival_date->format('d M Y') }}</span></div>@endif
        </div>
        @endif
...
<div class="grid-2">
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fas fa-box" style="margin-right:.5rem;color:#2d6a4f;"></i> Consignments ({{ $shipment->consignments->count() }})</h3>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fas fa-file-alt" style="margin-right:.5rem;color:#e8a838;"></i> ASN (Advance Shipping Notice)</h3>
        </div>
        <div class="card-body">
            @if($shipment->asn)
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:1rem;">
                <div style="padding:.6rem;background:#fef3c7;border-radius:8px;">
                    <div style="font-size:.65rem;color:#92400e;font-weight:600;">ASN Number</div>
                    <div style="font-weight:800;font-family:monospace;">{{ $shipment->asn->asn_number }}</div>
                </div>
                <div style="padding:.6rem;background:#fef3c7;border-radius:8px;">
                    <div style="font-size:.65rem;color:#92400e;font-weight:600;">Status</div>
                    <div><span class="badge {{ $shipment->asn->status==='pricing_done'?'badge-success':'badge-warning' }}">{{ ucfirst(str_replace('_',' ',$shipment->asn->status)) }}</span></div>
                </div>
                <div style="padding:.6rem;background:#f8fafc;border-radius:8px;">
                    <div style="font-size:.65rem;color:#64748b;font-weight:600;">Total Items</div>
                    <div style="font-weight:700;">{{ $shipment->asn->total_items }}</div>
                </div>
                <div style="padding:.6rem;background:#f8fafc;border-radius:8px;">
                    <div style="font-size:.65rem;color:#64748b;font-weight:600;">Generated</div>
                    <div style="font-size:.82rem;">{{ $shipment->asn->generated_at?->format('d M Y H:i') ?? '—' }}</div>
                </div>
            </div>
...
<div class="card" style="margin-top:1.25rem;">
    <div class="card-header">
        <h3><i class="fas fa-list" style="margin-right:.5rem;color:#1e3a5f;"></i> Product Items</h3>
    </div>
...
<div class="card" style="margin-top:1.25rem;">
    <div class="card-header">
        <h3><i class="fas fa-clipboard-check" style="margin-right:.5rem;color:#16a34a;"></i> GRN Received</h3><a href="{{ route('logistics.grn.show', $shipment->grn) }}" class="btn btn-outline btn-sm">View GRN</a>
    </div>
    <div class="card-body">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:.75rem;">
            <div style="padding:.5rem;background:#dcfce7;border-radius:8px;text-align:center;">
                <div style="font-size:.65rem;color:#166534;font-weight:600;">GRN #</div>
                <div style="font-weight:700;font-family:monospace;">{{ $shipment->grn->grn_number }}</div>
            </div>
            <div style="padding:.5rem;background:#f8fafc;border-radius:8px;text-align:center;">
                <div style="font-size:.65rem;color:#64748b;font-weight:600;">Received</div>
                <div style="font-weight:700;">{{ $shipment->grn->total_items_received }}</div>
            </div>
            <div style="padding:.5rem;background:#f8fafc;border-radius:8px;text-align:center;">
                <div style="font-size:.65rem;color:#64748b;font-weight:600;">Damaged</div>
                <div style="font-weight:700;color:#dc2626;">{{ $shipment->grn->damaged_items }}</div>
            </div>
            <div style="padding:.5rem;background:#f8fafc;border-radius:8px;text-align:center;">
                <div style="font-size:.65rem;color:#64748b;font-weight:600;">Receipt Date</div>
                <div style="font-weight:600;font-size:.82rem;">{{ $shipment->grn->receipt_date->format('d M Y') }}</div>
            </div>
        </div>
    </div>
</div>
