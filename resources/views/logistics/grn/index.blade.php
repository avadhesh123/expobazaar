...
<div class="card" style="margin-bottom:1.25rem;border-color:#fde68a;">
    <div class="card-header" style="background:#fffbeb;">
        <h3><i class="fas fa-exclamation-triangle" style="margin-right:.5rem;color:#e8a838;"></i> Shipments Pending GRN ({{ $pendingShipments->count() }})</h3>
    </div>
...
<div class="card" style="margin-bottom:1.25rem;">
    <div class="card-body" style="padding:.85rem 1.4rem;">
        <form method="GET" id="grnFilterForm" action="{{ route('logistics.grn') }}" style="display:flex;flex-wrap:wrap;gap:.75rem;align-items:flex-end;">
            ...
            // Default to warehouse 1 if company is 2000 and no selection yet
            if (empty($selectedId) && request('company_code') === '2000') {
            $selectedId = 1;
            }
            if (empty($selectedId) && request('company_code') === '2100') {
            $selectedId = 2;
            }
            if (empty($selectedId) && request('company_code') === '2200') {
            $selectedId = 3;
            }
            @endphp
            <div style="min-width:140px;"><label style="font-size:.7rem;font-weight:600;color:#64748b;display:block;margin-bottom:.25rem;">Warehouse</label>
                <select name="warehouse_id" id="warehouseSelect" style="width:100%;padding:.4rem .5rem;border:1px solid #d1d5db;border-radius:8px;font-size:.82rem;font-family:inherit;">
                    <option value="">All</option>
                    @foreach($warehouses as $wh)
                    <option
                        value="{{ $wh->id }}"
                        {{ $selectedId == $wh->id ? 'selected' : '' }}>
                        {{ $wh->name }}
                    </option>
                    @endforeach
                </select>
...
<div class="card">
    <div class="card-header">
        <h3><i class="fas fa-clipboard-check" style="margin-right:.5rem;color:#2d6a4f;"></i> All GRNs</h3><span style="font-size:.78rem;color:#64748b;">{{ $grns->total() }} records</span>
    </div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr data-company-code="{{ $grn->company_code }}">   
                    ...
                    <td style="text-align:center;font-weight:600;color:#166534;">{{ $grn->excess_items }}</td>
                ...
                @empty
                <tr>
                    <td colspan="11" style="text-align:center;padding:3rem;color:#94a3b8;"><i class="fas fa-clipboard-check" style="font-size:2rem;display:block;margin-bottom:.5rem;"></i>No GRNs found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
