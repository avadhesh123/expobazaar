...
class LiveSheetItemChange extends Model
{
    ...
    {
        return $this->belongsTo(LiveSheetItem::class, 'live_sheet_item_id');
    }
    ...
    public static function trackChanges(LiveSheetItem $item, array $newDetails, User $user, string $role, ?string $reason = null): int
    {
        ...
        // Format role with user email: "vendor:abc@gmail.com"
        $roleWithEmail = $role . ':' . ($user->email ?? 'unknown');
        ...
        foreach ($trackedFields as $field) {
            ...
            $oldStr = $oldVal !== null ? trim((string) $oldVal) : null;
            $newStr = $newVal !== null ? trim((string) $newVal) : null;

            // SKIP if new value is not filled (null, empty string, or '0')
            if ($newStr === null || $newStr === '' || $newStr === '0' || $newStr === '0.00') {
                continue;
            }

            // Normalize old empty to null for comparison
            if ($oldStr === '' || $oldStr === '0' || $oldStr === '0.00') {
                $oldStr = null;
            }
