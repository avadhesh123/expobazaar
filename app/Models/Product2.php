...
class Product extends Model
{
    ...
    protected $fillable = [
        'sku', 'name', 'description', 'category_id', 'vendor_id', 'company_code',
    ...
    public function liveSheetItems() { return $this->hasMany(LiveSheetItem::class); }

    public function scopeByCompanyCode($query, $code) { return $query->where('company_code', $code); }
    public function scopeListed($query) { return $query->where('status', 'listed'); }
    public function scopeAvailable($query) { return $query->where('stock_quantity', '>', 0); }
