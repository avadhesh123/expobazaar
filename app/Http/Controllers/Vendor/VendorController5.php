...
class VendorController extends Controller
{
    ...
    public function submitKyc(Request $request)
    {
        ...
        $request->validate([
            ...
            'bank_swift_code'         => 'required|string|max:20',
            ...
            'documents.signed_contract'   => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:10240',
            'documents.iec_certificate'   => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:10240',
            'documents.msme_certificate'  => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:10240',
        ...
        $docs = [];
        $docTypes = ['gst_certificate', 'cancelled_cheque', 'signed_contract', 'iec_certificate', 'msme_certificate'];
        foreach ($docTypes as $type) {
            if ($request->hasFile("documents.{$type}")) {
                ...
                \App\Models\VendorDocument::updateOrCreate(
                    ...
                    ['document_name' => ucfirst(str_replace('_', ' ', $type)), 'file_path' => $path, 'file_type' => $file->getMimeType(), 'file_size' => $file->getSize(), 'uploaded_by' => auth()->id(), 'status' => 'pending']
                );
            }
        }
        ...
        if ($request->hasFile('documents.other')) {
            foreach ($request->file('documents.other') as $file) {
                ...
                \App\Models\VendorDocument::create([
                    ...
                    'uploaded_by' => auth()->id(), 'status' => 'pending',
                ]);
            }
        }
    ...
    {
        $vendor = auth()->user()->vendor;
        if ($liveSheet->vendor_id !== $vendor->id) abort(403);
    ...
    public function uploadLiveSheet(Request $request, LiveSheet $liveSheet)
    {
        ...
        $vendor = auth()->user()->vendor;
        if ($liveSheet->vendor_id !== $vendor->id) abort(403);
        if ($liveSheet->is_locked) return back()->with('error', 'Live sheet is locked.');
        ...
        try {
            ...
            foreach ($sheet->getRowIterator(1, 1) as $row) {
                foreach ($row->getCellIterator() as $cell) {
                    ...
                    if (str_contains($val, 'vendor sku') || $val === 'sku') $colMap['vendor_sku'] = $col;
                    elseif (str_contains($val, 'sap')) $colMap['sap_code'] = $col;
                    elseif (str_contains($val, 'barcode')) $colMap['barcode'] = $col;
                    elseif (str_contains($val, 'product name')) $colMap['product_name'] = $col;
                    elseif (str_contains($val, 'description')) $colMap['description'] = $col;
                    elseif (str_contains($val, 'hsn') || str_contains($val, 'hts')) $colMap['hsn_code'] = $col;
                    elseif (str_contains($val, 'duty %') || str_contains($val, 'duty%')) $colMap['duty_percent'] = $col;
                    elseif (str_contains($val, 'product length')) $colMap['length'] = $col;
                    elseif (str_contains($val, 'product width')) $colMap['width'] = $col;
                    elseif (str_contains($val, 'product height')) $colMap['height'] = $col;
                    elseif (str_contains($val, 'product weight') || str_contains($val, 'weight') && str_contains($val, 'gram')) $colMap['weight_grams'] = $col;
                    elseif (str_contains($val, 'material') && !str_contains($val, 'other')) $colMap['material'] = $col;
                    elseif (str_contains($val, 'other material')) $colMap['other_material'] = $col;
                    elseif (str_contains($val, 'color')) $colMap['color'] = $col;
                    elseif (str_contains($val, 'finish')) $colMap['finish'] = $col;
                    elseif (str_contains($val, 'sub category')) $colMap['sub_category'] = $col;
                    elseif (str_contains($val, 'category') && !str_contains($val, 'sub')) $colMap['category'] = $col;
                    elseif (str_contains($val, 'qty in inner')) $colMap['qty_inner_pack'] = $col;
                    elseif (str_contains($val, 'inner') && str_contains($val, 'length')) $colMap['inner_length'] = $col;
                    elseif (str_contains($val, 'inner') && str_contains($val, 'width')) $colMap['inner_width'] = $col;
                    elseif (str_contains($val, 'inner') && str_contains($val, 'height')) $colMap['inner_height'] = $col;
                    elseif (str_contains($val, 'qty in master')) $colMap['qty_master_pack'] = $col;
                    elseif (str_contains($val, 'master') && str_contains($val, 'length')) $colMap['master_length'] = $col;
                    elseif (str_contains($val, 'master') && str_contains($val, 'width')) $colMap['master_width'] = $col;
                    elseif (str_contains($val, 'master') && str_contains($val, 'height')) $colMap['master_height'] = $col;
                    elseif (str_contains($val, 'master') && str_contains($val, 'weight')) $colMap['master_weight_kg'] = $col;
                    elseif (str_contains($val, 'qty offered')) $colMap['qty_offered'] = $col;
                    elseif (str_contains($val, 'vendor fob')) $colMap['vendor_fob'] = $col;
                    elseif (str_contains($val, 'target fob')) $colMap['target_fob'] = $col;
                    elseif (str_contains($val, 'final qty')) $colMap['final_qty'] = $col;
                    elseif (str_contains($val, 'final fob')) $colMap['final_fob'] = $col;
                    elseif (str_contains($val, 'freight factor')) $colMap['freight_factor'] = $col;
                    elseif (str_contains($val, 'wsp factor')) $colMap['wsp_factor'] = $col;
                    elseif (str_contains($val, 'comment')) $colMap['comments'] = $col;
                    elseif (str_contains($val, 's.no') || $val === 'sno') $colMap['sno'] = $col;
                }
            }
