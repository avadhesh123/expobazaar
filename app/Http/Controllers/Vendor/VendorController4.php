...
class VendorController extends Controller
{
     ...
     */
    public function downloadLiveSheetTemplate1(LiveSheet $liveSheet)
    {
        $vendor = auth()->user()->vendor;
        if ($liveSheet->vendor_id !== $vendor->id) {
            abort(403);
...
public function downloadLiveSheetTemplate(LiveSheet $liveSheet)
{
    ...
    $headers = [
        ...
        'Product Description (Min 100 words)', 'Hsn & Hts Code', 'Duty %',
        'Product Length (Inches)', 'Product Width (Inches)', 'Product Height (Inches)',
        'Product Weight (Gram)', 'Material Composition', 'Other Material', 'Color',
        'Product Finish', 'Category', 'Sub Category', 'Qty In Inner Pack',
        'Inner Carton Length (Inches)', 'Inner Carton Width (Inches)',
        'Inner Carton Height (Inches)', 'Qty In Master Pack',
        'Master Carton Length (Inches)', 'Master Carton Width (Inches)',
        'Master Carton Height (Inches)', 'Master Carton Weight (Kg)',
        'Qty Offered (Units/Sets)', 'Vendor FOB Mumbai ($)', 'Target FOB ($)',
        'Final Qty (Units/Sets)', 'Total No Of Master Cartons', 'Master Carton CBM',
        'CBM Shipment', 'Final FOB ($)', 'Duty', 'Freight Factor', 'Freight',
        'Landed Cost', 'WSP Factor', 'WSP ($)', 'Comments',
    ];
    ...
    $headerStyle = [
      //  'font'      => ['bold' => true, 'color' => ['argb' => 'FFFFFFFF']],
        'fill'      => ['fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startColor' => ['argb' => 'FFFFECB3']],
    ...
    // Auto-size first few key columns; set a default width for the rest
    foreach (range(1, count($headers)) as $colIndex) {
        $colLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($colIndex);
        $sheet->getColumnDimension($colLetter)->setWidth($colIndex <= 5 ? 20 : 16);
    }

    // Stream as .xlsx download
    $filename = "live-sheet-{$liveSheet->live_sheet_number}.xlsx";
    ...
    {
        $path = public_path('downloads/Live_Sheet_US.xlsx');
        if (!file_exists($path)) {
