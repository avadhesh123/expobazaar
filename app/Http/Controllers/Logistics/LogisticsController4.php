...
class LogisticsController extends Controller
{
    ...
    public function inventoryAgeing(Request $request)
    {
        ...
        // GRN ageing — days since receipt per GRN
        $grnAgeing = Grn::with('shipment', 'warehouse')
            ->when($companyCode, fn($q, $v) => $q->where('company_code', $v))
            ->latest()->get()->map(function ($grn) {
                $grn->ageing_days = now()->diffInDays($grn->receipt_date);
                return $grn;
            });

        // Inventory ageing grouped by warehouse
        $byWarehouse = Inventory::with('product', 'warehouse')
            ->when($companyCode, fn($q, $v) => $q->where('company_code', $v))
            ->where('quantity', '>', 0)
            ...
            ->map(function ($group) {
                ...
                return [
                    ...
                    'total'     => $items->count(),
                    'buckets'   => [
                        '0-30'   => $items->where('ageing_days', '<=', 30)->count(),
                        '31-60'  => $items->whereBetween('ageing_days', [31, 60])->count(),
                        '61-90'  => $items->whereBetween('ageing_days', [61, 90])->count(),
                        '91-180' => $items->whereBetween('ageing_days', [91, 180])->count(),
                        '180+'   => $items->where('ageing_days', '>', 180)->count(),
                    ],
                ];
            });

        return view('logistics.inventory.ageing', compact('grnAgeing', 'byWarehouse', 'companyCode'));
    }

    public function warehouseAllocation(Request $request)
    {
        $inventoryByWarehouse = Inventory::with('product.vendor', 'warehouse')
            ->when($request->company_code, fn($q, $v) => $q->where('company_code', $v))
            ->where('quantity', '>', 0)->get()
            ->groupBy('warehouse_id')
            ->map(fn($group) => [
        ...
        $movements = InventoryMovement::with('product', 'fromWarehouse', 'toWarehouse')
