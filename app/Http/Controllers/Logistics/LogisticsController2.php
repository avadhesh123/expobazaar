...
class LogisticsController extends Controller
{
    ...
    public function downloadAsn(\App\Models\Asn $asn)
    {
        ...
        $csv = "ASN Number,{$asn->asn_number}\nShipment,{$asn->shipment->shipment_code}\nCompany,{$asn->company_code}\nDate,{$asn->generated_at}\n\n";
        $csv .= "SKU,Product,Vendor,Quantity,CBM\n";
        foreach ($items as $item) {
            $csv .= implode(',', [
                $item['sku'] ?? '', '"' . ($item['name'] ?? '') . '"', $item['vendor_id'] ?? '',
                $item['quantity'] ?? '', $item['cbm'] ?? '',
            ]) . "\n";
        }

        return response($csv, 200, [
            'Content-Type' => 'text/csv',
            'Content-Disposition' => "attachment; filename=\"ASN-{$asn->asn_number}.csv\"",
        ]);
    }
